...
$(document).ready(function() {
		...
		$('.select').each(function() {
			...
			if ($this.closest('.modal').length > 0) {
				...
				$this.select2({
					...
					dropdownParent: $this.closest('.modal .input-block'), // Append dropdown to the modal
				});
			} else {
				// Regular behavior for select2 on the page
				$this.select2({
					minimumResultsForSearch: 0, // Enable search
					width: '100%' // Full width for the dropdown
				});
	...
	if ($('.bookingrange').length > 0) {
		...
		$('.bookingrange').val('MM/DD/YYYY - MM/DD/YYYY');
	}
	...
	/*$(document).on('click', '.save-prospect-stage', function(e) {
    //e.preventDefault(); // Prevent the default form submission

		// Get form field values
		let stageName = $('#stage_name').val().trim();
		...
		if (createdDate === '') {
			$('#created_date').addClass('is-invalid');
			$('#created_date').next('.invalid-feedback').show();
			isValid = false;
		}

		// If the form is valid, submit it
		if (isValid) {
			$('#frmprospectstage').submit();
		}
	});*/

	// On Save & Next Button Click
	$(".add-info-fieldset .wizard-next-btn").on('click', function () {
		// Get the current fieldset
		var currentFieldset = $(this).closest('fieldset');

		// Validate the form within the fieldset
		var isValid = true;
		...
		currentFieldset.find('input').each(function() {
			if (!this.checkValidity()) {
				...
				$(this).siblings('.invalid-feedback').show(); // Show the error message
			} else {
				$(this).removeClass('is-invalid');
				$(this).siblings('.invalid-feedback').hide(); // Hide the error message
			}
		});

		// Validate select fields
		currentFieldset.find('select').each(function() {
			if (!this.value) { // Check if the select field has no value selected
				isValid = false;
				$(this).addClass('is-invalid'); // Add Bootstrap invalid class
				$(this).siblings('.invalid-feedback').show(); // Show the error message
			} else {
		...
		currentFieldset.find('input[type="email"]').each(function() {
			if (!this.checkValidity()) {
				isValid = false;
				$(this).addClass('is-invalid'); // Add Bootstrap invalid class
				$(this).siblings('.invalid-feedback').show(); // Show the error message
		...
		currentFieldset.find('input[type="date"]').each(function() {
			if (!this.checkValidity()) {
				isValid = false;
				$(this).addClass('is-invalid'); // Add Bootstrap invalid class
				$(this).siblings('.invalid-feedback').show(); // Show the error message
		...
		if (isValid) {
			...
			currentFieldset.next().fadeIn('slow');
			currentFieldset.css({
				'display': 'none'
			});
			...
			// Scroll to the first invalid input or select if form is invalid
			/*$('html, body').animate({
				scrollTop: currentFieldset.find('.is-invalid').first().offset().top - 100
			}, 500);*/
		}
	});
	
	// Back button functionality
	$(".add-info-fieldset .wizard-pre-btn").on('click', function () {
		$(this).closest('fieldset').prev().fadeIn('slow');
		$(this).closest('fieldset').css({
			'display': 'none'
		});
		// Update progress bar for back navigation
		$('.progress-bar-wizard .active').removeClass('active').addClass('activated').prev().addClass('active');
	});
	
	


	// Contact Wizard 02-10-2024
